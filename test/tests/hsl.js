...
let rgb_alpha_implied = IS_CULORI | IS_RGB | ALPHA_IMPLIED;
let hsl_alpha_implied = IS_CULORI | IS_HSL | ALPHA_IMPLIED;

console.log(IS_CULORI, IS_HSL, ALPHA_IMPLIED, hsl_alpha_implied);
...
tape('to_hsl works correctly', function(test) {
	...
	test.deepEqual(to_hsl({ r: 0, g: 0, b: 0 }), { s: 0, l: 0, flags: hsl_alpha_implied }, 'black');

	test.deepEqual(to_hsl({ r: 0.25, g: 0.25, b: 0.25 }), { s: 0, l: 0.25, flags: hsl_alpha_implied }, 'R = G = B yields undefined hue');
	test.deepEqual(to_hsl({ r: 0.6, g: 0.6, b: 0.6 }), { s: 0, l: 0.6, flags: hsl_alpha_implied }, 'R = G = B yields zero saturation');
...
tape('culori.hsl() parses hsl / hsla CSS strings', function(test) {
	...
	test.deepEqual(culori.hsl('hsl(0, 100%, 0%)'), { h: 0, s: 0, l: 0, flags: hsl_alpha_implied }, 'black');
	test.deepEqual(culori.hsl('hsl(100, 0%, 50%)'), { h: 100, s: 0, l: 0.5, flags: hsl_alpha_implied }, 'grey');
